...
const getMyChats = TryCatch(async (req, res, next) => {
  ...
  const transformedChats = chats.map(({ _id, name, members, groupChat }) => {
    ...
    return {
      _id,
      groupChat,
      //sirf 3 avatar dp me dikhana h
      avatar: groupChat
        ? members.slice(0, 3).map(({ avatar }) => avatar?.url || "")
        : [otherMember?.avatar?.url || ""],
      name: groupChat ? name : otherMember?.name,
      members: members.reduce((prev, curr) => {
        //except me all members
...
const leaveGroup = TryCatch(async (req, res, next) => {
  ...
  emitEvent(req, ALERT, chat.members,{ message: `User ${user.name} has left the group`,
    chatId}
  );
...
const renameGroup = TryCatch(async (req, res, next) => {
  ...
  if (chat.creator.toString() !== req.user.toString())
    return next(new ErrorHandler("You are not allowed to rename the group"));
...
const getMessages = TryCatch(async (req, res, next) => {
  ...
  //dont allow to read chat if mem is exited from the group
  const chat = await Chat.findById(chatId);
  if(!chat) return next(new ErrorHandler("Chat not found", 404));

  if(!chat.members.includes(req.user.toString())) return (new ErrorHandler("You are not allowed to access this chat", 404))
